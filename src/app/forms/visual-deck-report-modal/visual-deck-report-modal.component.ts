...
export class VisualDeckReportModalComponent implements OnInit {
  ...
  ngOnInit() {
    this.visualDeckReportModalForm = this.formBuilder.group({
      ...
      signsOfLeaks: [this.data.rowsMap?.get('visualsignsofleak') ?? false, (this.data.rowsMap?.get('unitUnavailable')) ? false : Validators.required],
      invasiveReviewRequired: [this.data.rowsMap?.get('furtherinvasivereviewrequired') ?? false, (this.data.rowsMap?.get('unitUnavailable')) ? null : Validators.required],
